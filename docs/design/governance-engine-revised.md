# Hephaestus as a User-Authored Workflow Governance Engine — Revised (post-audit)

> **Origin of this file.** Derived from `governance-engine.md` (the post-grill design, 2026-06-23)
> with changes applied from `architecture-audit.md` (2026-06-23). The original `governance-engine.md`
> is left intact; this is the audit-revised copy. **Diff vs the original:**
> - a first-class **Provider** concept + **registry** (audit #1) so a new provider — `gemini_cli` or a direct API — is one registered module, not edits in six places;
> - per-provider raw→turn **event mapping moved onto the Provider** (audit #3);
> - **`ExecutionContract` recast as a *derived* record** produced by a Node→Contract **resolution** module (audit #2);
> - a note to **lift the glossary into `CONTEXT.md`** (audit #4).
> Audit-driven additions are tagged inline as *(audit #N)*.

**Status:** Design — revised 2026-06-23
...
- **Engine** — a `{provider, model, effort}` config a node runs on. Stateless from Hephaestus's view; holds no cross-node memory.
- **Provider** *(audit #1)* — a pluggable **adapter** (Claude / Codex / Gemini / a direct API) registered once, bundling: a runner, raw→turn `normalize_event`, contract→flags, model discovery, and `owns_model`. Adding a provider = register one module.
...
- **Engine** — `{provider, model, effort}` referenced from the catalog; the node's executor names one. Swappable/A-B-able under a node.
- **Provider** *(audit #1)* — the pluggable adapter the engine resolves to, behind a **registry**. Interface: `{runner, normalize_event, flags(contract), discover_models(), owns_model(model)}`. Routing/catalog/`service` read the registry; the closed `Tool` enum becomes registry keys; per-provider raw→turn mapping lives on the Provider *(audit #3)*. A new provider — CLI or direct API — is **one module, registered** (the `AgentRunner` protocol already abstracts CLI/SDK/HTTP execution styles).
...
- *(audit #2)* The contract is produced by a **Node→ExecutionContract resolution** module — one place that resolves `(Node + input artifacts + constitution)` into the governed run spec (a `resolve(...)` interface), rather than the current ad-hoc 16-field construction in `service`. The contract is the *derived record*, not an authored data-bag.

### 7.2 Executor model + UI discriminator
A node carries `executor: {kind, …}`:
- `kind: "engine"` → `{provider, model, effort}`; at run time `provider` resolves to a registered **Provider** adapter *(audit #1)*. Rendered as a **provider-colored box** with the model label.
...
- *(audit #1)* **Provider registry** — providers as registered adapters behind one interface; closed `Tool` enum → registry keys; per-provider event mapping on the Provider. Enables `gemini_cli` + direct APIs.
- *(audit #2)* **`ExecutionContract` as a derived record** via a Node→Contract resolution module.

## 9. Follow-ups from the audit (non-ADR)

- *(audit #4)* Lift the glossary (§2/§7) into **`CONTEXT.md`** so the domain vocabulary is the project glossary (AI-navigability + naming locality).
- See `architecture-audit.md` for the full deepening rationale (deletion-test analysis) behind §-tags above.
