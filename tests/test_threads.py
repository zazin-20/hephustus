...
def _make_thread(db_path, agent_id="arch-001"):
    # ensure a profile row exists so FK is satisfied
    with connect(db_path) as conn:
        conn.execute(
            "INSERT OR IGNORE INTO profiles(agent_id,name,role,rules,created_at) VALUES(?,?,?,?,?)",
            (agent_id, "Arch", "architect", "[]", "2026-01-01T00:00:00Z"),
        )
        conn.commit()
    return get_or_create_thread(db_path, agent_id=agent_id, name="thread-a")
